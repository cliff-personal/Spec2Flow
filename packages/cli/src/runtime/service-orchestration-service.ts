...
export interface DeterministicServiceEvidenceArtifact {
  ...
  path: string;
  category: 'service-startup' | 'service-health' | 'other';
  contentType?: string;
}
...
async function waitForHealthChecks(checks: TopologyHealthCheck[], cwd: string): Promise<boolean> {
  if (checks.length === 0) {
    return true;
  }
...
async function startServiceProcess(command: string, cwd: string, logPath: string): Promise<void> {
...
export async function runServiceOrchestration(options: RunServiceOrchestrationOptions): Promise<DeterministicServiceOrchestrationResult> {
  ...
  for (const serviceName of orderedServices) {
    ...
    const initiallyHealthy = await waitForHealthChecks(checks, options.cwd);
    if (initiallyHealthy) {
      writeJsonFrom(options.cwd, healthArtifactPath, buildServiceHealthArtifact(serviceName, healthArtifactPath, checks, true));
      summaries.push({
        name: serviceName,
        status: 'ready',
    ...
    if (!startCommand) {
      writeJsonFrom(options.cwd, healthArtifactPath, buildServiceHealthArtifact(serviceName, healthArtifactPath, checks, false));
      summaries.push({
        name: serviceName,
        status: 'failed',
        ...(checks[0]?.target ? { healthTarget: checks[0].target } : {}),
    ...
    try {
      await startServiceProcess(startCommand, options.cwd, logPath);
      artifacts.push({
      ...
      const healthyAfterStart = await waitForHealthChecks(checks, options.cwd);
      writeJsonFrom(options.cwd, healthArtifactPath, buildServiceHealthArtifact(serviceName, healthArtifactPath, checks, healthyAfterStart));
      artifacts.push({
        id: `service-health-${sanitizeToken(serviceName)}`,
        kind: 'report',
        path: healthArtifactPath,
