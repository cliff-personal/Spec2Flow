...
| P0 | Governed publication execution | `packages/cli/src/runtime/collaboration-publication-service.ts`, `packages/cli/src/platform/platform-publication-service.ts`, `packages/cli/src/platform/platform-control-plane-action-service.ts`, git provider integration surfaces | turn publication from record-only behavior into deterministic commit / PR creation / merge-orchestration execution | collaboration becomes a real autonomous endpoint instead of a documentation artifact | `npm run build`, targeted publication tests, smallest publish-path regression |
