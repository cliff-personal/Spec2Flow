...
| Project adapter profile | AI runtime capability is part of project registration and scheduling truth | `register-platform-project`; `POST /api/projects`; `packages/cli/src/platform/platform-project-service.ts`; `packages/cli/src/platform/platform-auto-runner-service.ts` | `gap` | Adapter runtime is still discovered from `.spec2flow/runtime/model-adapter-runtime.json` instead of a first-class project profile with scheduler-visible capabilities |
