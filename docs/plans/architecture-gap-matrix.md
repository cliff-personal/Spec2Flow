...
- Status: active
- Source of truth: `docs/architecture.md`, `docs/agent-orchestration-platform-design.md`, `package.json`, `packages/cli/src/planning/task-graph-service.ts`, `packages/cli/src/runtime/execution-state-service.ts`, `packages/cli/src/runtime/task-result-service.ts`, `packages/cli/src/runtime/workflow-loop-service.ts`, `packages/cli/src/runtime/collaboration-publication-service.ts`, `packages/cli/src/adapters/adapter-runner.ts`, `schemas/task-graph.schema.json`, `schemas/execution-state.schema.json`, `schemas/model-adapter-runtime.schema.json`
...
## Shortest Path To Automatic PR

| Priority | Capability Track | Current Evidence | Status | Why It Blocks Full Autonomy |
| --- | --- | --- | --- | --- |
| `P0` | Shared scheduler hardening | `packages/cli/src/platform/platform-scheduler-service.ts`; `packages/cli/src/platform/platform-worker-service.ts`; `packages/cli/src/cli/run-platform-worker-task-command.ts` | `partial` | Workers can execute the loop, but lease safety, replay safety, and single-writer guarantees are not yet strong enough for unattended multi-run operation |
| `P0` | Evaluator-owned acceptance | `packages/cli/src/planning/task-graph-service.ts`; `packages/cli/src/runtime/task-result-service.ts`; `packages/cli/src/adapters/adapter-runner.ts`; `schemas/evaluation-summary.schema.json` | `partial` | Evaluation now controls accept vs reject vs reroute-to-repair, including precise repair-target inference from `findings` and `nextActions`, but publication and final completion still need broader controller-owned hardening across the full platform path |
| `P0` | Closed-loop repair enforcement | `packages/cli/src/runtime/auto-repair-policy-service.ts`; `packages/cli/src/runtime/task-result-service.ts`; `packages/cli/src/platform/platform-auto-repair-service.ts` | `partial` | Repair routing exists, but the controller still lacks hard invariants for when repair must rerun versus when a route may escalate or terminate |
| `P0` | Governed publication execution | `packages/cli/src/runtime/collaboration-publication-service.ts`; `packages/cli/src/platform/platform-publication-service.ts`; `packages/cli/src/platform/platform-control-plane-action-service.ts` | `partial` | Publication artifacts and persistence exist, but the full PR publication contract is not yet consistently sequenced after evaluator acceptance and operator policy |
| `P0` | Human-on-exception control plane | `packages/cli/src/platform/platform-control-plane-server.ts`; `packages/cli/src/platform/platform-observability-service.ts`; `packages/web` | `partial` | The backend can expose runs and actions, but the operator surface is still not a complete exception-only control plane for approvals, repair escalation, and publication governance |
| `P1` | Intake, observability, and project isolation | `packages/cli/src/platform/platform-project-service.ts`; `packages/cli/src/platform/platform-observability-service.ts`; `packages/cli/src/platform/platform-project-adapter-profile.ts` | `partial` | These matter for scale and operator trust, but they do not block the first autonomous PR loop once P0 control authority is correct |
...
| Review policy and approval gates | Human approval stays in policy and blocks collaboration when required | `reviewPolicy` is generated into task graph and claims; `packages/cli/src/runtime/task-result-service.ts` blocks collaboration completion when a schema-backed handoff is still `awaiting-approval` and blocks evaluation when acceptance is not granted | `implemented` | Approval is enforced for collaboration and evaluation artifacts, but the richer approval-record lifecycle is still shallow |
| Platform event model and observability | A PostgreSQL-backed control plane can reconstruct run, task, repair, publication, and approval state from durable events and read models | `get-platform-run-state`; `get-platform-observability`; `packages/cli/src/platform/platform-event-taxonomy.ts`; `packages/cli/src/platform/platform-observability-service.ts`; `packages/cli/src/platform/platform-worker-service.ts`; `packages/cli/src/platform/platform-publication-service.ts`; `packages/cli/src/platform/platform-auto-repair-service.ts` | `partial` | Durable events and read models now exist, but there is still no streaming transport, operator-facing API, or web UI control plane |
...
The shortest-path order to the first unattended automatic PR loop is now:

1. `P0` shared scheduler hardening
2. `P0` evaluator-owned acceptance
3. `P0` closed-loop repair enforcement
4. `P0` governed publication execution
5. `P0` human-on-exception control plane
6. `P1` intake, observability, and project isolation
...
- added a self-dogfood deterministic runtime at `.spec2flow/runtime/model-adapter-runtime.deterministic.json`
- the self-dogfood `.spec2flow/runtime/model-adapter-runtime.json` can now delegate deterministic stages through `stageRuntimeRefs` while keeping Copilot CLI as the default runtime for the rest of the loop
